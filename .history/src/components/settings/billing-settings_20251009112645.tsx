...
const BillingSettings = async (props: Props) => {
  ...
  // If planFeatures are not found, we can still render the card,
  // but let's keep the existing early return for now for features dependency.
  if (!planFeatures) return null // Changed to return null for a cleaner output

  // --- Content for Card 2 (Payment Method Card) ---
  ...
  // --- Content for Card 3 (Upgrade Plan Card) ---
  ...
  return (
    <div className="mt-8 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Modal
          ...
          trigger={
            <Card className="shadow-lg p-0 h-full cursor-pointer hover:shadow-xl transition-shadow">
                {/* We use the custom paymentMethodCard content as the trigger */}
                {paymentMethodCard} 
            </Card>
          }
        >
          {/* Content for the payment details modal (e.g., a form to change card) */}
          <div className="p-4">
              <p>Payment details view/edit form will go here.</p>
              {/* This is where you might put a form to edit the card */}
          </div>
        </Modal>


        {/* --- Card 3: Upgrade/Change Plan --- */}
        <Modal
          title="Choose A Plan"
        ...
        >
          {/* Modal Content for Subscription Form */}
