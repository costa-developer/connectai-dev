...
import { useLocation } from "react-router-dom"; // Assuming you use react-router-dom for routing

const Sidebar = () => {
  const [isOpen, setIsOpen] = useState(false); // mobile toggle
  const location = useLocation(); // To get the current path for active state
  ...
  return (
    ...
    >
          ...
          {menuItems.map((item) => {
            ...
            return (
                <li key={item.name}>
                  <a
                    href={item.href}
                    // 2. Implemented distinct active link styling
      ...
      <div className="mx-4 my-4 relative">
        {/* 3. Improved card text readability by removing the redundant white background on inner elements */}
        <div
